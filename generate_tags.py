#!/usr/bin/env python3
"""Genererar custom HTML-taggsidor som matchar undersidornas design."""
import os

CONTENT_DIR = "content"

TAG_MAP = {
    "algorithm": [("treehouse", "Varför finns det så ont om bilder av kojor byggda av barn?")],
    "avatarthelastaibender": [("avatar", "Character arcs of Avatar")],
    "businessmodels": [("businesschmodels", "Business model enshitification")],
    "characterarc": [("avatar", "Character arcs of Avatar")],
    "comedian": [("anthony", "Jag gillar verkligen Anthony Jesilnek")],
    "cv": [("jag", "Fragment")],
    "designprocess": [("treehouse", "Varför finns det så ont om bilder av kojor byggda av barn?")],
    "digitalgararden": [("varfor-digital-garden", "Varför digital garden?")],
    "enshitification": [("businesschmodels", "Business model enshitification"), ("treehouse", "Varför finns det så ont om bilder av kojor byggda av barn?")],
    "likeablepersons": [("anthony", "Jag gillar verkligen Anthony Jesilnek")],
    "meta": [("gastbok", "Gästbok"), ("jag", "Fragment")],
    "premiss": [("premiss", "Vikten av att ha en enastående premiss")],
    "story-arc": [("premiss", "Vikten av att ha en enastående premiss")],
    "storytelling": [("premiss", "Vikten av att ha en enastående premiss"), ("avatar", "Character arcs of Avatar")],
    "webb": [("varfor-digital-garden", "Varför digital garden?")],
}

TAG_TEMPLATE = r"""<!DOCTYPE html>
<html lang="sv">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>__TAG_DISPLAY__ — Ola Janson</title>
    ...
    <style>
        @font-face { font-family: 'Magic Romance'; src: url('/fonts/magic-romance.ttf') format('truetype'); }
        @font-face { font-family: 'Fraunces'; src: url('/fonts/fraunces.ttf') format('truetype'); }
        @font-face { font-family: 'SKF Sans'; src: url('/fonts/skfsans.otf') format('opentype'); }
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        body {
            ...
            background-image: repeating-linear-gradient(0deg, transparent, transparent 3px, rgba(0,0,0,0.08) 3px, rgba(0,0,0,0.08) 4px) !important;
        ...
        .title-band { width: 100%; padding-top: 80px; padding-bottom: 0; }
        ...
        .search-button {
            ...
            background: #0e0e0e; border: 1px solid #2a2a2a; border-radius: 16px;
            color: #888; padding: 4px 12px;
            font-family: 'SKF Sans', sans-serif; font-size: 12px; cursor: pointer;
        }
        .search-button svg { width: 12px; height: 12px; stroke: #888; fill: none; }
        .darkmode-btn {
            background: none; border: none; cursor: pointer;
            padding: 3px; display: flex; align-items: center; margin-left: auto;
        }
        ...
        .tag-article-list { list-style: none; }
        ...
        .tag-article-list li a:hover { color: #F2D024; }
        ...
        @media (max-width: 900px) {
            ...
            .title-band .col-left, .title-band .col-right { display: none; }
            .title-band .col-main { padding: 0 16px; }
            .page-title { font-size: clamp(2.2rem, 8vw, 3.5rem) !important; }
            .topbar-controls { width: auto; }
            .page-layout { grid-template-columns: 1fr; grid-template-areas: "main" "right"; }
...
<div class="title-band">
    <div class="grid-row">
        ...
        <div class="col-main">
            <h1 class="page-title">__TAG_DISPLAY__</h1>
        </div>
...
<div class="topbar">
    <div class="topbar-inner">
        <div class="topbar-controls">
            ...
            <button class="language-toggle" aria-label="Växla språk">
                <span class="lang-sv active" style="opacity:1;color:#EAE6DA;">SV</span><span class="lang-separator">/</span><span class="lang-en">EN</span>
            </button>
        </div>
    </div>
</div>
...
<div class="page-layout">
    ...
    <main class="main-col">
        ...
        <p class="tag-label">Sidor med taggen</p>
...
def make_articles_html(articles):
    return "\n            ".join(
        f'<li><a href="/{slug}">{title}</a></li>' for slug, title in articles
    )


os.makedirs(os.path.join(CONTENT_DIR, "tags"), exist_ok=True)

for tag_slug, articles in sorted(TAG_MAP.items()):
    tag_display = "#" + tag_slug
    articles_html = make_articles_html(articles)
    html = (
        TAG_TEMPLATE
        .replace("__TAG_DISPLAY__", tag_display)
        .replace("__ARTICLES_HTML__", articles_html)
        .replace("__TAG_SLUG__", tag_slug)
    )
    out_path = os.path.join(CONTENT_DIR, "tags", f"{tag_slug}.html")
    with open(out_path, "w") as f:
        f.write(html)
    print(f"✓ {out_path}")

print("Taggsidor klara!")
